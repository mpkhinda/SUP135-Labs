...
```{r map test}
# test plot to check that streets, nodes, and tracts are using 
...
for (i in seq(1, 5, 1)){
  touch_list <- st_touches(streets,nodes %>% subset(OBJECTID == i)) # get list of binary variables for all road segments indicating if the segment touches the node
  nodes_example$node_degree[i] <- sum(unlist(touch_list)) # roll up count of 1s for roads that touch the node to obtain the node degree
}
